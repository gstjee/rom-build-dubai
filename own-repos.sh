...
# frameworks/base
if [[ ! -d "$F_B_DIR" ]]; then
git clone https://github.com/crdroidandroid/android_frameworks_base.git "$F_B_DIR"
...
if [[ ! -d "$P_A_S_DIR" ]]; then
git clone https://github.com/crdroidandroid/android_packages_apps_Settings.git "$P_A_S_DIR"
...
git clone https://github.com/crdroidandroid/android_packages_apps_crDroidSettings.git "$P_A_crDS_DIR"
...
if [[ ! -d "$V_M_D_DIR" ]]; then
git clone --depth=5 https://github.com/gstjee/vendor_motorola_dubai-mid.git "$V_M_D_DIR"
cd "$V_M_D_DIR" && git remote add gstjee https://github.com/gstjee/vendor_motorola_dubai-mid.git
cd "$BASE_DIR"
fi

# device/motorola/sm7325-common
if [[ ! -d "$D_M_SM_DIR" ]]; then
git clone https://github.com/gstjee/device_motorola_sm7325-common-mid.git "$D_M_SM_DIR"
...
if [[ ! -d "$D_M_D_DIR" ]]; then
git clone https://github.com/gstjee/device_motorola_dubai-mid.git "$D_M_D_DIR"
